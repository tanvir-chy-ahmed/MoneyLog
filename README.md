# 💸 MoneyLog — Personal Expense Tracker App

**MoneyLog** is a simple, fast, and intuitive **expense tracker app** designed to help you manage your daily spending and stay financially organized.  

---

## ✨ Features
- ➕ Add **income & expense** entries  
- 📊 View spending history clearly  
- 🗓️ Track **daily, weekly & monthly** expenses  
- 📱 Minimal and **user-friendly UI**  
- ⚡ **Fast and lightweight**  
- 🔒 **Local data storage** for privacy  

---

## 🛠️ Tech Stack
- **Language:** Kotlin  
- **UI:** Jetpack Compose  
- **Architecture:** MVVM  
- **State Management:** ViewModel + State  
- **Platform:** Android  

---

---
## 📸 Screenshots

<div style="display: flex; flex-wrap: wrap; gap: 10px; justify-content: center;"> 
  <img src="https://github.com/user-attachments/assets/c5455471-1db1-487d-95bc-0b011cdca86f" alt="SS1" width="300" />
  <img src="https://github.com/user-attachments/assets/9cf96a16-4beb-457a-8a7e-181b90e55f60" alt="SS2" width="300" />
  <img src="https://github.com/user-attachments/assets/3444fcf9-3e97-4a4c-87f2-00c2ed3fc3ad" alt="SS3" width="300" />
  <img src="https://github.com/user-attachments/assets/70a5f7d7-e99e-4f48-9959-19f5a56ea186" alt="SS4" width="300" />
  <img src="https://github.com/user-attachments/assets/bec17709-dc33-4ffd-b5af-1a108aa7fd36" alt="SS5" width="300" />
  <img src="https://github.com/user-attachments/assets/4c6de91f-2524-4014-a46a-880dbd879ff5" alt="SS6" width="300" />
  <img src="https://github.com/user-attachments/assets/6086dde8-b349-4f73-b5fb-30728078c042" alt="SS7" width="300" />
  <img src="https://github.com/user-attachments/assets/a2e2a53f-38b2-4d7f-891f-6e4476f32d01" alt="SS8" width="300" />
</div>
---

---

## 🚀 Getting Started
Clone the project:
```bash
git clone https://github.com/tanvir-chy-ahmed/MoneyLog.git
cd MoneyLog
```


🎯 Future Improvements

📈 Expense analytics & charts

☁️ Cloud backup (Firebase)

🔔 Budget alerts & notifications

🌙 Dark mode

📤 Export expenses (CSV / PDF)

🤝 Contributions

Contributions, issues, and feature requests are welcome!
Feel free to fork, star ⭐, and submit pull requests.

🧑‍💻 Author

Tanvir Ahmed
Jetpack & Flutter App Developer
GitHub

# 📄 License
This project is MIT Licensed. See LICENSE for details.

---







